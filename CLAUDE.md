...
- `src/settings.js` owns the live `extension_settings.IF_Image` object. UI handlers mutate it and invoke the supplied save callback. Defaults are deep-merged without overwriting user values, so a new key with a plain default does not need a migrator. `src/migration.js` applies sequential in-place migrations (currently v10; v9 unifies `generation.checkpoint` and `backends.a1111.checkpoint` into one selection, Backends value winning; v10 re-keys `checkpointProfiles` from checkpoint title to unique profile ids (`cp1`, `cp2`, …), stamps each row's `checkpoint`/`name`, and derives `backends.a1111.activeProfileId` from the checkpoint match), with `CURRENT_VERSION` derived from the migrator array length; add a migrator only when an existing value must be rewritten.
...
Pipeline (all modules exist and are wired): `src/runtime/events.js` detects markers in eligible rendered text → `src/runtime/marker-pipeline.js` (restore-from-IDB check first, then optional LLM rewrite in Assist/Full mode, then compile) → `src/runtime/tasks.js` FIFO queue (concurrency 1, maxQueued 20, timeout 300000 ms) → `src/runtime/executor.js` dispatches to a backend client → `src/runtime/insert.js` renders slot states, the toolbar (View/Regen/Edit/Delete — Repro was removed in D14; the seed-reuse `regenerate({seed})` path remains for the gallery), and the lightbox → `src/storage/images.js` persists the record before the slot swaps. Invariants: slot/entry identity is the rendered marker text, never the post-rewrite prompt; known revisions are deduplicated on restore; **the executor call is `execute(taskSnapshot, signal)`, not `execute(taskSnapshot, { signal })`**; task specs are projected into declared snapshot fields, so do not assume arbitrary extra properties survive; `MESSAGE_DELETED` carries the remaining chat length, not a deleted message ID.

Prompt side: `parseTriggers` resolves roster/style references and returns characters, modifiers, styles, a dialect override, param overrides (size/steps/cfg/seed, size keywords), and residual scene text. `assemblePrompt` produces `{ prompt, negative, params }`. Profile keys are `krea2`, `anima`, and `illustrious`; compiler dialect keys are `krea`, `anima`, and `illus`. Profiles describe prompt style and generation defaults, not an actual server checkpoint. `src/backends/checkpoint-profiles.js` is pure. Since D14, `checkpointProfiles` is keyed by unique profile id (`cp1`, `cp2`, …), each row `{name, checkpoint, profile, params…}`, so one checkpoint can hold many named profiles; a row exists only when the user clicks "Save profile" in the Settings tab (saving under a changed name creates a new row) — discovery never seeds rows. `backends.a1111.activeProfileId` selects THE profile driving generation (`getActiveProfile`); the legacy `generation.checkpoint`/`backends.a1111.checkpoint` keys stay synced to the active row's checkpoint for executor fallback. `suggestCheckpointProfile` fills the editor's starting values (server per-model defaults when `/internal/models` enrichment ran, else PROFILES numbers; sampler/scheduler spelling aligned with the discovered lists), `normalizeCheckpointProfile` clamps the saved row (requires a non-empty `checkpoint`; blank `name` falls back to it), `SIZE_PRESETS`/`matchSizePreset` back the size preset select, and `mergeParams` applies the five-layer precedence PROFILES < generation.params[profile] < checkpointProfiles[profileId] < marker overrides < LLM overrides (`checkpointTitle` only stamps `params.checkpoint`).
...
Reading all files and implementation code in `st-chatu8/` is explicitly permitted for research, including UI structure, popup behavior, styles, and integration flows. This permission is for reading and analysis only: do not copy or port its implementation into this repository, as the user's licensing restriction on copying remains in effect. For other extensions, do not read or copy implementation without explicit user permission. Implement required behavior independently using this repository's existing modules. Keep code and comments in English and match the plain ESM/JSDoc style.
