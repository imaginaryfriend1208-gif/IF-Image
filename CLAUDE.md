...
- `src/settings.js` owns the live `extension_settings.IF_Image` object. UI handlers mutate it and invoke the supplied save callback. Defaults are deep-merged without overwriting user values, so a new key with a plain default does not need a migrator. `src/migration.js` applies sequential in-place migrations (currently v7), with `CURRENT_VERSION` derived from the migrator array length; add a migrator only when an existing value must be rewritten.
